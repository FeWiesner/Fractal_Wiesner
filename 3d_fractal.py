...
#cambiar archivo
network = ['HANO_','BALE_', 'FOSO_', 'MODE_']
side =['derecho_', 'izquierdo_', 'knee_']
criterion = ['HGL', 'd', 'Q']
...
for net in network:
    for s in side:
        for crit in criterion:
            ...
            with open(net+s+crit+'.txt') as csv_file:
                ...
                for row in csv_reader:
                    if crit == 'HGL':
                        ...
                        b = 100
                    if net == 'HANO_' or net =='FOSO_':
                        a = 100
                        fine = 16j
                        X.append(float(row[0])/a)
                        Y.append(float(row[1])/a)
                        Z.append(float(row[2])*b)
                    elif net == 'MODE_':
                        a = 100
                        fine = 70j
                        X.append((float(row[0])-1640000)/a)
                        Y.append((float(row[1])-4940000)/a)
                        Z.append(float(row[2])*b)
                    else: 
                        a = 1
                        fine = 90j
